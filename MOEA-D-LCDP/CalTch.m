function Tch = CalTch(PopObj,W,Region,Z,Sub)
...
%     PBI(Sub) = d1 + 5*d2;
    Tch = zeros(1,size(PopObj,1));
%   Tch(Sub)= max(abs(PopObj(Sub,:)-Z)./W(Region(Sub),:),[],2);
    Tch(Sub)= max(abs(PopObj(Region(Sub),:)-Z)./W(Region(Sub),:),[],2);
end