function [return_pop,return_Fitness] = UpdateP1(Population,N,VAR)
...
% This function is written by Jinlong Zhou
    ...
    if isempty(fPopulation)%
        ifFitness = CalFitness(ifPopulation.objs,ifPopulation.cons,0); % ifPopulation.cons
        Next2 = ifFitness < 1;
        if sum(Next2) <= N
            [~,Rank] = sort(ifFitness);
            Next2(Rank(1:N )) = true;
    ...
    elseif length(fPopulation) <N
        ...
        %
%         if ~isempty(ifPopulation)
         ifFitness = CalFitness(ifPopulation.objs,ifPopulation.cons,0); %
         Next2 = ifFitness < 1;
%         end
